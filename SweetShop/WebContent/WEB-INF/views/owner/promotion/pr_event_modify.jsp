...
<script>
	...
	$(document).ready(function(){
		...
		var attr2 = '${event_detail}';
		var attr3 = '${event_img_list}';
		console.log("=============");
		console.log(attr);
		console.log(attr2);
		console.log(attr3);
...
<script>
	var start_day = '${event_detail.event_start_day}';
	var end_day = '${event_detail.event_end_day}';
	var tab = '${event_detail.event_kind}';
	var target = '${event_detail.event_discount_target}';
	var discount_way = '${event_detail.event_discount_way}';
	var event_discount = '${event_detail.event_discount}';

	$(function() {
		console.log(">>");
		console.log(target.event_discount_target);

		tabChange(tab);

		$("#event_subject").keyup(function() {
			eventTitleChange();
		});

		$("#event_desc_input").keyup(function() {
			eventDescChange();
		});
		...
		if(selected_type_tab =="A"){
			$("#pum").val(target);
			$("#dc_value").val(event_discount);
			selectTypeDC(discount_way);

		}else if(selected_type_tab == "B"){
			$("#pum2").val(target);
			$("#service_value").val(event_discount);
			selectTypeService(discount_way);

		}

		<c:forEach items="${event_img_list}" var="item">
			var obj = new Object();
			obj.img_path='${item.img_path}';
			obj.img_org_name='${item.img_org_name}';
			obj.event_order='${item.event_order}';
			obj.img_file_name='${item.img_file_name}';
			img_array.push(obj);
		</c:forEach>

		item_index = img_array.length;
	...
	function saveCall() {
		...
		if(confirm("수정하시겠습니까?")){
			...
			var url = "/shop_event_update.json";
			var pum="";
			var value="";
			if(selected_type_tab =="A"){
				pum = $("#pum").val();
			...
			console.log(selected_type_tab+","+pum +","+value+","+selected_type+","+event_desc_input); */
	
			var comment = newLineChange($('#event_desc_input').val());
	
			$.post(url, {
				event_seq:'${event_detail.event_seq}',
	...
	function deleteCall() {
		...
		if(confirm("이벤트를 삭제하시겠습니까?")){
			...
			var eventSeq = '${event_detail.event_seq}';
			var url = "/shop_event_delete.do";
			
			$.get(url,{"event_seq":eventSeq}).done(function(data){
			...
			<ul>
				...
				<li class="left">
					...
					<div class="cont_area ">
						<div class="ev_regis_subject">
							...
							<input type="text" id="event_subject" class="regis_input_blur" placeholder=" 이벤트 명을 입력하여 주세요."
									value="${event_detail.event_title}">
						</div>
					</div>
					...
					<div class="cont_area ev_regis_cont mt20">
						<ul class="tap_menu">
							<li id="A" class="on"><a href="javascript:tabChange('A')">할인 이벤트</a><li>
							<li id="B" ><a href="javascript:tabChange('B')">혜택 제공 이벤트</a><li>
						</ul>


						<div class="gab_box">
							<div class="tab1">
								<label for="pum">할인 이벤트 품목</label>
								<input type="text" id="pum" class="regis_input_blur" placeholder="할인 이벤트 품목을 입력해 주세요.">
							</div>

							<div class="w_half whleft tab1">
								<label for="typeDC">할인 방식</label>
								<input type="text" id="typeDC" class="regis_input_blur bg_arrow"  onclick="suggest(1);" style="background: url('/src/owner/images/contents/arrow_btm.png') no-repeat 98% center;"
										value="" placeholder="방식을 선택해 주세요.        "><!-- 반응형웹 사이즈 문제로 화살표가 들어가는 인풋화면에서는 플레이스홀더 옆에 8칸 비워주세요 !! //-->
									...
									//-->
									 ...
									 <div class="business_type" id="suggest_1" style="display:none;">
										<ul>
											<li><a href="javascript:selectTypeDC('M')" class="close"><span class="stile_vh">할인방식</span>금액할인</a></li>
											<li><a href="javascript:selectTypeDC('P')" class="close"><span class="stile_vh">할인방식</span>%할인</a></li>
										</ul>
									 </div>
									 <script type="text/javascript">
							...
							<div class="w_half whright tab1" >
								<label for="dc_value" >할인 입력</label>
								<input type="text" id="dc_value" class="regis_input_blur"  placeholder="할인방식 선택 후 입력해 주세요.">
							</div>

							<div class="tab2" style="display:none">
								<label for="pum2">적용 서비스 품목</label>
								<input type="text" id="pum2" class="regis_input_blur"  placeholder="혜택이 적용될 서비스 품목을 입력해 주세요."style="font-size:14px;">
							</div>

							<div class="w_half whleft tab2" style="display:none">
								<label for="typeService">제공 혜택</label>
								<input type="text" id="typeService" class="regis_input_blur bg_arrow"  onclick="suggest(2);" style="width:92%; padding-right:8%; background: url('/src/owner/images/contents/arrow_btm.png') no-repeat 98% center;" placeholder="서비스 품목 제공">
									<!-- 레이어팝업으로 나오는 화면에서 선택시 인풋값에 자동 들어가도록 프로그램 처리 필요//-->
									<div class="business_type" id="suggest_2" style="display:none;">
										<ul>
											<li><a href="javascript:selectTypeService('O')" class="close"><span class="stile_vh">제공혜택</span>1+1</a></li>
											<li><a href="javascript:selectTypeService('T')" class="close"><span class="stile_vh">제공혜택</span>2+1</a></li>
											<li><a href="javascript:selectTypeService('U')" class="close"><span class="stile_vh">제공혜택</span>사이즈업</a></li>
											<li><a href="javascript:selectTypeService('S')" class="close"><span class="stile_vh">제공혜택</span>서비스 품목 제공</a></li>
										</ul>
									 </div>
									 <script type="text/javascript">
										$(".close").click(function() {
											$("#suggest_2").css("display","none");
										});
							...
							<div class="w_half whright tab2"  style="display:none">
								<label for="service_value" class="hide">제공 혜택 직접입력</label>
								<input type="text" id="service_value" class="regis_input_blur"  placeholder="제공되어질 서비스 품목을 입력해주세요.">
							</div>
								...
								<div class="cont">
									<div class="item_cont_area">
										...
										<img src="/src/owner/images/contents/icon_camera.png"
											onError="this.src='/src/owner/images/contents/icon_camera.png';"
											onclick='document.all.file.click();'/>
										</p>
										<form id="frm" action="/event_img_upload.do" method="post" encType="multipart/form-data">
							...
							<div class="design_textarea">
								...
								<textarea  class="write_input_blur" id="event_desc_input" >${event_detail.event_detail_desc}</textarea>
								 <script type="text/javascript">
										 $(document).ready(function() {
											...
											$('.design_textarea').find( 'textarea' ).keyup();
										});
						...
						//-->
							...
							<div class="btn_regis_area">
								...
								<p class="cancle"><a href="javascript:deleteCall()">삭제</a></p>
								<p class="cancle"><a href="/pr_event_list.do">취소</a></p>
							</div>

							<div class="btn_regis_area" style="border-top:1px solid #eee; margin-top: 20px; padding-top:30px;">
								<p class="cancle"><a href="/pr_event_list.do">목록</a></p>
							</div>
						</div>
