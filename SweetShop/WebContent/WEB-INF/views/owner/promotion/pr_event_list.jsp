...
<script type="text/javascript">
	$(document).ready(function(){
		...
		var attr = '${event_list}';
		var gp = '${event_list.getCurrentPage()}';
...
<div id="container">
	...
	<!-- 우측 콘텐츠의 시작 //-->
	<div id="content_area">
		<h3><img src="/src/owner/images/contents/h3_img_02.png" alt=""  /><span>홍보ㆍ마케팅 상품 &nbsp; &gt; &nbsp; </span> 이벤트</h3>
		...
		<div class="guide_area_wide">
			...
			<ul>
				<li class="searchzone">
					...
					<p class="sform">
						<input type="text" id="search_input" class="topSearch_blur" value="${param.event_name}">
						<span class="a_area"><a href="javascript:search(1)"></a></span>
					</p>
					<script type="text/javascript">
		...
		<div class="cont_area">
			...
			<col width="*" />
				...
				<c:if test="${event_list.getTotalRow() eq '0'}">
					<tbody class="result">
						<tr>
							<td colspan="7">
				...
				<c:if test="${event_list.getTotalRow() ne '0'}">
					<tfoot>
						<tr>
							<th colspan="7">
								<ul class="borad_paging">
									<li class="paging_info">${event_list.getSPage()}-${event_list.getEPage()}  &nbsp; of &nbsp; ${event_list.getTotalRow()}</li>
									<li class="paging_first"><a href="javascript:search(${event_list.getBlockFirstPage()})"><img src="/src/owner/images/board/btn_first.png" /></a></li>
									<li class="paging_prev"><a href="javascript:search(${event_list.getPrePage()})"><img src="/src/owner/images/board/btn_prev.png" /></a></li>
									<li class="paging_now"><input type="text" placeholder="1"   value="${event_list.getCurrentPage()}" class="paging" ></li>
									<li class="paging_next"><a href="javascript:search(${event_list.getNextPage()})"><img src="/src/owner/images/board/btn_next.png" /></a></li>
									<li class="paging_end"><a href="javascript:search(${event_list.getBlockLastPage()})"><img src="/src/owner/images/board/btn_end.png" /></a></li>
								</ul>
							</th>
						</tr>
					</tfoot>
					...
					<c:forEach items="${event_list}" var="event_list">
						<tbody onclick="movePage(${event_list.event_seq})" class="bgcolor">
							<tr>
								<th><fmt:formatDate value="${event_list.reg_date}" type="both" pattern="yyyy.MM.dd"/></th>

									<c:set var="status" value="${event_list.event_status}" />
									<c:choose>
								...
								<td class="subject">${event_list.event_title}</td>
								<td class="ev_date">${event_list.event_start_day}~${event_list.event_end_day}</td>
								<td class="report">
									<c:if test="${event_list.success eq 's'}">
										<span><a href="/pr_event_index.do?event_seq=${event_list.event_seq}"><img src="/src/owner/images/contents/icon_medal.png" /></a></span>
									</c:if>
									<span><a href="/pr_event_index.do?event_seq=${event_list.event_seq}"><img src="/src/owner/images/contents/icon_report.png" /></a></span>
								</td>
							</tr>
						</tbody>
					</c:forEach>
		...
		<div class="list_btm_more_area">
			...
			<p class="more_green"><a href="/pr_event_write.do"><img src="/src/owner/images/contents/btn_more_plus.png" /></a></p>
		</div>
