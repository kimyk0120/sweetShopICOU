...
// 		console.log("$(.board_list).height() : " , $(".board_list").height() );
	...
	function lastPostFunc(page){
		...
		$.ajax({
			  method: "POST",
			  url: "/user/search_result_scroll_load.ajax",
...
<style>
	html {height:auto;}
	#wrap_wide{padding-bottom: 0%;}
	div.photo img{height:7em;}
</style>
<input type="hidden" value="${searchText}" id="searchText" ></input>

	
	<!-- 검색결과없을때 -->
	<c:if test="${empty storeList}">
	 	<div class="search_result_not">
			<img src="/src/user/images/common/icon_icou_noevent.png" />
			<p>아이쿠!<br />검색결과가 없습니다.</p>
			다른 검색어로 찾아보시겠어요?
		</div> 
	</c:if>
